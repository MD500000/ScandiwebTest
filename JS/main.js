...
if (isStale(myButton2)) {
  ...
  function isAnyFormVisible() {
    return $('#DVDForm, #DVDdesc, #FurnitureForm, #FurnitureDesc, #BookForm, #BookDesc').is(':visible');
  }

  $('.main-navbar').on('click', '#SaveButton', function(event) {
    if (!isAnyFormVisible()) {
      // Show an alert if no product forms are visible
